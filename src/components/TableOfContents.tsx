...
const TableOfContents = ({ headers }) => {
  ...
  return (
    <div id='toc' class='sticky top-16 flex flex-col gap-2 pt-8'>
      {headers?.map((h) => (
        <a id={h.slug} className={`${h.slug !== headerToHighlight && 'opacity-30'} text-lg font-semibold mb-4 overflow-hidden text-ellipsis`} style={`padding-left: ${h.depth * 1.5}rem;`} href={`#${h.slug}`}>
          {h.text}
        </a>
      ))}
    </div>
